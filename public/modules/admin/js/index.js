...
function(e) {
	...
	r = function(e) {
		...
		// 	}
		// 	"console" === i.pageType ? o() : r.render("layou1t").done(function() {
		// 		o(),
	...
	//分页点击跳转
	d('.layui-laypage-btn').on('click', function(){
		var url = d(this).data('url'),
			page =  Number(d(this).prev().val());
	...
	//分页选择页数
	d('.layui-table-page .layui-laypage .layui-laypage-limits select').on('change', function(){
		var paginate = d(this).val(),
			url = d(this).data('url');
