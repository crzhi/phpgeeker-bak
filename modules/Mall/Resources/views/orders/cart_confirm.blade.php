        ...
        <div class="layui-row">
            ...
            <div class="layui-col-sm12 house-usershop">
								...
								<tbody>
									@if(count($cartItems))
										@foreach($cartItems as $item)
											<tr class="item-box" data-id="{{ $item->productSku->id }}">
												<td align="center">
												...
												<td align="center">
													<div class="layui-table-cell">
														<span class="calculate">￥{{ $item->productSku->price * $item->amount }}</span>
													</div>
												</td>
												<td align="center">
													<div class="layui-table-cell">
														<p class="order-sku-totals">￥<span class="order-sku-total">{{ $item->productSku->price * $item->amount }}</span></p>
													</div>
												</td>
											</tr>
										@endforeach
				...
				<div class="house-usershop-table-num layui-form order-confrim-total">
					...
					<button class="layui-btn layui-order-pay-btn" data-order="{{ route('mall.orders.store') }}" data-href="">去支付</button>
				</div>
            </div>
        </div>
    </div>
